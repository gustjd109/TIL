...
2. 기본형과 참조형 데이터 타입의 차이
    ...
    - 참조형
        - 할당이나 연산 시, 값이 담긴 주솟값들로 이루어진 묶음을 가리키는 주솟값 복제<br><br>

## 2. 데이터 타입에 관한 배경지식
1. 메모리와 데이터
    - 비트(Bit)
        - 0 또는 1만 표현할 수 있는 하나의 메모리 조각
        - 각 비트는 고유한 식별자(Unique Identifier)로 위치 확인
            - 비트 단위로 위치를 확인하는 것은 비효율적
    - 바이트(Byte)
        - 8개의 비트로 구성
        - 1바이트는 총 256(2의 8승), 2바이트는 비트 16개이므로 65536(2의 16승)개의 값을 표현
        - 시작하는 비트의 식별자로 위치 확인 → 모든 데이터는 바이트 단위의 식별자인 메모리 주솟값(Memory Address)을 통해 서로 구분하고 연결
        - 바이트 단위로 위치를 확인하면, 표현 가능한 값이 늘어나고 검색 시간도 감소하지만 낭비 비트가 발생할 가능성이 있음
    - 자바스크립트의 메모리 공간 관리
        - 숫자의 경우 정수형인지 부동소수형인지를 구분하지 않고 64비트(8바이트)를 확보하여, 데이터 형변환을 걱정해야 하는 상황이 덜 발생<br><br>

2. 식별자와 변수
    - 변수(Variable)
        - 변할 수 있는 데이터
    - 식별자(Identifier)
        - 변수명 → 어떤 데이터를 식별하는 데 사용하는 이름<br><br>
...
1. 변수 선언
    - 변수 선언 예제
        ...
        ```
        - 변할 수 있는 데이터이므로 선언할 때는 undefined 타입
    - 변수 선언에 대한 메모리 영역의 변화
        - 메모리에서 빈 공간(임의로 @1003) 확보
...
2. 데이터 할당
    ...
    - 데이터 할당에 대한 메모리 영역의 변화
        ...
        - 확보한 공간의 식별자를 a로 지정
        - 데이터 영역에서 데이터를 저장하기 위한 빈 공간(임의로 @5004) 확보
        - 변수 영역에서 a라는 식별자를 검색(@1003)
        - 앞서 저장한 문자열의 주소(@5004)를 @1003의 공간에 대입
    - 왜 변수 영역에 값을 직접 대입하지 않고 한 단계를 더 거치는가
        - 데이터 변환을 자유롭게 할 수 있게 함과 동시에 메모리를 더욱 효율적으로 관리하기 위함
        - 자바스크립트에서 문자열은 정해진 규격이 없으며, 각각 필요한 메모리 용량과 글자 수가 가변적
            - 미리 확보한 공간 내에서만 데이터 변환을 할 수 있다고 가정한 경우
                ...
                - 즉, 효율적으로 문자열 데이터의 변환을 처리하려면 변수와 데이터를 별도의 공간에 나누어 저장하는 것이 효율적
...
1. 불변값
    ...
    - 숫자와 문자열을 예로 든 불변성의 개념
        ...
        b = 7;
        ```
        - 변수 a에 문자열 'abc'를 할당했다가 뒤에 'def'를 추가하면 새로운 문자열 'abcdef'를 만들어 그 주소를 변수 a에 저장
        - 변수 b에 숫자 5를 할당
            - 데이터 영역에서 5를 찾고, 없으면 데이터 공간을 하나 만들어 저장 후 그 주소를 b에 저장
        - 변수 c에 이미 만들어놓은 값이 있으니 그 주소를 재활용하여 5를 할당
        - 기존에 저장된 7이 없으므로 새로 만들어 변수 b에 저장
...
2. 가변값
    - 참조형 데이터의 기본적인 성질은 가변값인 경우가 많음
        - 설정에 따라 변경 불가능한 경우도 있음(Object.defineProperty, Object.freeze 등)
        - 불변값으로 활용하는 방안도 존재
    - 참조형 데이터의 할당
        ```javascript
        var obj1 = {
    ...
    - 참조형 데이터의 프로퍼티 재할당
        ...
        obj1.a = 2;
        ```
        - 데이터 영역에서 숫자 2가 없으므로 빈 공간인 @5005에 2를 저장하고, 이 주소를 @7103에 저장
...
3. 변수 복사 비교
    ...
    - 변수 복사 이후 값 변경 괄과 비교(객체의 프로퍼티 변경)
        ```javascript
        var a = 10;
        var b = a;
        var obj1 = {c: 10, d: 'ddd'};
        ...
        - 코드로 표현하면 아래와 같음
            ...
            - 단, 기본형은 주솟값을 복사하는 과정이 한 번만 이뤄지고, 참조형은 한 단계를 더 거치게 된다는 차이가 있는 것
    - 변수 복사 이후 값 변경 결과 비교(객체 자체를 변경했을 경우)
        ```javascript
        var a = 10;
        var b = a;
        var obj1 = {c: 10, d: 'ddd'};
...
1. 불변 객체를 만드는 간단한 방법
    ...
    - 객체의 가변성에 따른 문제점
        ...
        ```
        - 14번째 줄에서 user 변수와 user2 변수가 서로 같지 않다는 조건이 성립하지 않아 15번째 줄의 내용이 출력 없이 통과
        - 정보가 바뀐 시점에 알림을 보내거나, 바뀌기 전 정보와 바뀐 후 정보의 차이를 보여주는 등의 기능을 구현하려면 코드 수정 필요
    - 객체의 가변성에 따른 문제점의 해결 방법
        ...
        ```
        - changeName 함수가 새로운 객체를 반환하도록 코드 수정
        - user와 user2는 서로 다른 객체이므로 변경 전과 후를 비교 가능
        - 단, changeName 함수는 새로운 객체를 만들면서 변경할 필요가 없는 기존 객체의 프로퍼티(gender)를 하드코딩으로 입력하는 미흡한 점이 존재
            - 대상 객체에 정보가 많을수록, 변경해야 할 정보가 많을수록 사용자가 입력하는 수고가 증가
    ...
    - 기존 정보를 복사해서 새로운 객체를 반환하는 함수(얕은 복사)
        ...
        - 프로토타입 체이닝 상의 모든 포로퍼티를 복사, getter/setter는 복사히지 않음, 얕은 복사만을 수행한다는 아쉬운 점이 존재
    - copyObject를 이용한 객체 복사
        ...
        console.log(user === user2); // false
        ```
        - copyObject 함수를 통해 간단하게 객체를 복사하고 내용 수정 가능
        - user 객체 내부 변경이 필요한 경우 copyObject 함수를 사용한다는 전제하에 user 객체가 불변 객체라고 볼 수 있음
            - 프로퍼티 변경을 할 수 없게 시스템적 제약을 거는게 안전
...
2. 얕은 복사(Shallow copy)와 깊은 복사(Deep copy)
    ...
    - 중첩된 객체에 대한 깊은 복사
        ...
        ```
        - urls 프로퍼티에 copyOject 함수를 실행한 결과를 할당
    ...
    - 객체의 깊은 복사를 수행하는 범용 함수
        ...
        ```
        - target이 객체인 경우 내부 프로퍼티들을 순회하며 copyObjectDeep 함수를 재귀적으로 호출하고, 객체가 아닌 경우 target을 그대로 지정
        - target !== null 조건을 덧붙인 이유
            - typeof 명령어가 null에 대해서도 'object'를 반환하는 자바스크립트 자체의 버그가 존재
    ...
    - JSON을 활용한 간단한 깊은 복사
        ...
        - 단, 메서드(함수)나 숨겨진 프로퍼티인 __proto__나 getter/setter 등과 같이 JSON으로 변경할 수 없는 프로피티들은 모두 무시
...
1. undefined
    ...
    - undefined와 배열
        ...
        ```
        - arr1과 arr2는 배열에 3개의 빈 요소를 확보했지만 각 요소는 어떤 값도, undefined조차도 할당돼 있지 않음
        - arr3은 리터럴 방식으로 배열을 생성하면서 각 요소에 undefined를 부여
        - '버어있는 요소'와 'undefined를 할당한 요소'는 출력 결과가 다름
...
2. null
    ...
    - null 사용 주의 점
        - typeof null이 object인 자바스크립트 자체 버그가 존재
    - undefined와 null의 비교
        ```javascript
        var n = null;
